...
from .db.crud_token_blocklist import crud_token_blocklist
from .schemas import TokenBlocklistCreate, TokenData
...
async def verify_token(token: str, db: AsyncSession) -> TokenData | None:
    ...
    """
    is_blocklisted = await crud_token_blocklist.exists(db, token=token)
    if is_blocklisted:
        return None
...
async def blocklist_token(token: str, db: AsyncSession) -> None:
    payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
    expires_at = datetime.fromtimestamp(payload.get("exp"))
    await crud_token_blocklist.create(db, object=TokenBlocklistCreate(**{"token": token, "expires_at": expires_at}))
